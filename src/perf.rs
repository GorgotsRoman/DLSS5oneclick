...
mod tests {
    ...
    fn needs_enough_samples() {
        let mut knobs = FeederKnobs::default();
        knobs.work_resolution = 85;
    ...
    fn nearest_neighbour_prefers_close_work() {
        ...
        let mut knobs = FeederKnobs::default();
        knobs.work_resolution = 70;
        let e = expected_fps(&samples, &knobs).unwrap();
        assert!(e.fps > 80.0, "got {}", e.fps);
        knobs.work_resolution = 100;
        let e = expected_fps(&samples, &knobs).unwrap();
