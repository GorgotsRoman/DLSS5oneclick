...
pub fn find_override(exe: &Path) -> Option<GameOverride> {
    for o in load_overrides() {
        if (!o.match_exe.is_empty() && stem_matches(exe, &o.match_exe))
...
pub fn apply_to_cfg_text(cfg: &str, o: &GameOverride) -> String {
    ...
    let mut set = |key: &str, value: String| {
        ...
        if let Some(i) = lines.iter().position(|l| l.starts_with(&prefix) || l.to_ascii_lowercase().starts_with(&prefix.to_ascii_lowercase())) {
